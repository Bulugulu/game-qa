 ...
 *
 * The runner (runner.ts) is framework-fixed; this file is your project's
...
export const adapter: Adapter = {
  ...
  events: {
    subscribe: async (page, channel, filter) => {
      ...
      await page.evaluate(({ channel, handle }) => {
    ...
    drain: async (page, h) => {
      return page.evaluate(({ handle, filter }) => {
        ...
        if (w.__qaEventBuffers) w.__qaEventBuffers[handle] = [];
  ...
  // state. Should be idempotent. Wrap each call in try/catch so a missing
  // primitive doesn't break the whole reset.
  //
