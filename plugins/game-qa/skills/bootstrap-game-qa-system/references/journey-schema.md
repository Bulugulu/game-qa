...
The contract between the journey author (`running-game-qa-pass`) and the runner (`runner.ts`). Journeys are **data**, not code. The runner executes them; the engineer never writes Playwright JS.
...
| `dom` | `dom/<moment>.json` — Playwright accessibility snapshot |
...
Subscribes (if not already) and polls `adapter.events.drain` until `seen.length >= filter.nth ?? 1`. Throws on timeout. The runner records the subscription handle keyed by `<tab>:<channel>`, so later captures with `at: [events]` and the same `event:` drain the same buffer.
...
Calls the probe every 150ms; evaluates the predicate against `{ state, ...binds }`. Predicates run in a `new Function(...)` sandbox with full JS semantics.
...
Predicates have the same sandbox as `waitForState:`. Bound values are available as `$name` in strings (interpolated before eval) and as `name` inside the predicate body.
...
Runs raw JS with `page` and `ctx` in scope. Use sparingly — every `js:` step is a place where the schema doesn't yet express what you need, and a candidate for adding a real step type or adapter primitive.
...
The reviewer (`reviewing-game-qa`) reads `output.json` plus the screenshot/dom/console artifacts to judge each case.
