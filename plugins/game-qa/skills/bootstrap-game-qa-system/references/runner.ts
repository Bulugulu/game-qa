...
async function runCapture(step: any, page: Page, ec: ExecCtx, tabName: string, idx: number, ctx: AdapterContext) {
  ...
  for (const kind of at) {
    if (kind === "state") {
      ...
      m.events = handle ? await adapter.events.drain(page, handle) : null;
    } else if (kind === "timing") {
...
async function runWaitForEvent(step: any, page: Page, ec: ExecCtx, ctx: AdapterContext) {
  ...
  while (Date.now() < deadline) {
    const events = await adapter.events.drain(page, ec.eventHandles[handleKey]);
    seen.push(...events);
    if (seen.length >= wantNth) return;
...
async function fileCapabilityGap(ec: ExecCtx, name: string, ns: "arrange" | "probe") {
  ...
  const ticket = {
    ...
    blocking: true,
    fixHint: `Add ${name} to qa/adapter.ts under the ${ns} namespace.`,
  };
